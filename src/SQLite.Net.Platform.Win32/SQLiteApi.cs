...
namespace SQLite.Net.Platform.Win32
{
    public class SQLiteApiWin32 : ISQLiteApi
    {
        ...
        public Result Open(string filename, out IDbHandle db, int flags, string zvfs)
        {
            ...
            Result r = SQLiteApiWin32Internal.sqlite3_open_v2(databasePathAsBytes, out dbPtr, flags, IntPtr.Zero);
		...
		{
			return SQLiteApiWin32Internal.sqlite3_threadsafe();
		}
        ...
        public IDbStatement Prepare2(IDbHandle db, string query)
        {
            var internalDbHandle = (DbHandle) db;
            IntPtr stmt;
            Result r = SQLiteApiWin32Internal.sqlite3_prepare16_v2(internalDbHandle.DbPtr, query, query.Length*2, out stmt, IntPtr.Zero);
            if (r != Result.OK)
            {
